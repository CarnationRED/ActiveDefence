...
public class ActiveDefenceTurretBehaviour : Jundroo.SimplePlanes.ModTools.Parts.PartModifierBehaviour
{
    ...
    IEnumerator Initialize()
    {
        ...
        if (!ServiceProvider.Instance.GameState.IsInDesigner)
        {
            ...
            ActiveDefenceTurretScript script = gameObject.AddComponent<ActiveDefenceTurretScript>();
            ...
            script.p_TargetingStyle = setting.TargetingStyle;
            int.TryParse(setting.ActionGroup, out int ag);
            script.p_ActionGroup = ag;
            script.p_TrackMissilesFirst = setting.AimMissilesFirst == "Yes";
        }
        Destroy(this);
    }
}